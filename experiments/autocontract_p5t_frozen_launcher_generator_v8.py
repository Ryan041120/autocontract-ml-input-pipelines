#!/usr/bin/env python3
"""Byte-exact write-once generator for the isolated attempt3-v4 launcher."""
from __future__ import annotations
import argparse, hashlib, json, os, subprocess, sys, tempfile
from pathlib import Path
from typing import Any
ROOT=Path(__file__).resolve().parents[1]
CANONICAL_TARGET=ROOT/"experiments"/"autocontract_p5t_frozen_launcher_v8.py"
CANONICAL_FREEZE=ROOT/"benchmark"/"final_v1"/"p5t_attempt3_freeze_v8.json"
...
def bindings_from_freeze(freeze: Path,expected_freeze_sha: str)->dict[str,Any]:
 ...
 _need(isinstance(raw,dict) and raw.get("schema_version")=="autocontract.p5t-attempt3-v4-freeze.v8" and raw.get("status")=="pass","freeze_shape")
 bindings=raw.get("bindings",{}); active=bindings.get("active_sources",{}); runtime=bindings.get("runtime",{})
 paths={"runner":ROOT/"experiments"/"autocontract_p5t_attempt3_v14.py","core":ROOT/"experiments"/"autocontract_p5t_benefit_surface.py","protocol":ROOT/"benchmark"/"final_v1"/"p5t_benefit_surface_protocol.json","result_schema":ROOT/"benchmark"/"final_v1"/"p5t_attempt3_result_v4.schema.json","generator":Path(__file__),"contracts":ROOT/"experiments"/"autocontract_p5t_attempt3_contracts_v5.py"}
...
def synthetic_selftest()->dict[str,Any]:
 ...
 with tempfile.TemporaryDirectory(prefix="p5t_launcher_v8_bytes_") as raw:
...
def main(argv: list[str]|None=None)->int:
 ...
 try:
  ...
  verified=subprocess.run([bindings["fixed_python"],"-B",bindings["runner_path"],"--mode","verify-freeze-v8","--freeze",str(args.freeze),"--expected-freeze-sha",args.expected_freeze_sha],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL,check=False)
